...
The selected tuning parameters are $\alpha$ = `r round(enet_alpha_min_cvm, 2)` and $\lambda_{1SE}$ = `r round(lambda_1se_enet, 2)`. The model with these parameters has a test error of `r round(test_error, 2)`. 
...
There are `r ncomp_cv |> unname()` components in the partial least squares model, with an MSPE of `r mspe`.
